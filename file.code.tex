...
\NewDocumentCommand{\ds@file@drawstructurerecord} {mm} {
	\pgfmathparse{\ds@file@kbytewidth/1024 * #2 - 0.1em}
	\edef\@nodewidth{\pgfmathresult pt}
	\node[file record h, text width = \@nodewidth] (r) {#1};
	\addtocounter{dsfbytenumber}{#2}
}

\NewDocumentCommand{\ds@file@drawstructureblocks} {} {
	\pgfmathsetmacro\@remainingbytes{int(mod(\thedsfbytenumber, 1024))}
	\ifnum\@remainingbytes=0\relax
		\pgfmathsetmacro\@lastblock{int(\thedsfbytenumber / 1024) - 1}
	\else
		\pgfmathsetmacro\@lastblock{int(\thedsfbytenumber / 1024)}
	\fi
	\node at (1,1) {\@remainingbytes, \@lastblock};
	\foreach \n in {0, ..., \@lastblock}{
		\draw [draw = red, xshift = -0.05em]
